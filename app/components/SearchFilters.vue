...
<template>
  <aside class="hidden lg:block w-64 shrink-0">
    <div class="sticky top-4 rounded-xl border border-neutral-200 flex flex-col overflow-hidden">
      <div class="px-4 py-3 border-b border-neutral-200">
        <h2 class="text-sm font-bold text-neutral-900">Filter By:</h2>
      </div>

      <div class="px-4 py-4 border-b border-neutral-200">
        <h3 class="text-sm font-bold text-neutral-900 mb-2">Your budget</h3>
      ...
      <div class="px-4 py-4 border-b border-neutral-200">
        <h3 class="text-sm font-bold text-neutral-900 mb-3">Rating</h3>
        <div class="flex flex-col gap-2">
          <label v-for="star in [5, 4, 3, 2, 1]" :key="star" class="flex items-center gap-2 text-sm cursor-pointer"
            :class="minRating === star ? 'text-primary-700' : 'text-neutral-700'
              ">
            ...
            <div class="flex">
              <UIcon v-for="i in 5" :key="i" name="i-lucide-star" class="size-4"
                :class="i <= star ? 'text-amber-400' : 'text-neutral-300'" />
            </div>
            <span>& up</span>
          </label>
        </div>
      ...
      <div class="px-4 py-4 border-b border-neutral-200">
        <h3 class="text-sm font-bold text-neutral-900 mb-3">Specials</h3>
        <div class="flex flex-col gap-2">
      ...
      <div class="px-4 py-4 border-b border-neutral-200">
        <h3 class="text-sm font-bold text-neutral-900 mb-3">Time of Day</h3>
      ...
      <div class="px-4 py-4">
        <h3 class="text-sm font-bold text-neutral-900 mb-3">Duration</h3>
        <div class="grid grid-cols-2 gap-2">
          <label v-for="option in durationOptions" :key="option.value"
            class="flex items-center gap-2 text-sm text-neutral-700 cursor-pointer">
      ...
      <button v-if="hasActiveFilters"
        class="px-4 py-3 border-t border-neutral-200 text-xs text-primary-600 hover:underline text-left"
        @click="emit('reset')">
        Reset all filters
      </button>
    </div>
